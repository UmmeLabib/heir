...
#include "tests/Examples/openfhe/ckks/db_min/db_min_4_lib.h"
...
// db.min of 4 raw values under real CKKS, with -7.5 appearing twice: the
// index tie-break must leave exactly one winning column, so the duplicate
// minimum still decrypts to -7.5 and not to a multiple or a fraction of it.
TEST(DbMin4Test, RunTest) {
  auto cryptoContext = db_min_4__generate_crypto_context();
  auto keyPair = cryptoContext->KeyGen();
  auto publicKey = keyPair.publicKey;
  auto secretKey = keyPair.secretKey;
  cryptoContext = db_min_4__configure_crypto_context(cryptoContext, secretKey);

  float raw[4] = {42.0f, -7.5f, 100.0f, -7.5f};  // minimum appears twice
  float rawExpected = -7.5f;
  ...
  std::printf("raw input   : [%.2f, %.2f, %.2f, %.2f]  (min tied twice)\n",
              raw[0], raw[1], raw[2], raw[3]);
  std::printf("normalized  : [%.4f, %.4f, %.4f, %.4f]\n", arg0[0], arg0[1],
              arg0[2], arg0[3]);
  std::printf("packed hall : slots 0..3 hold the values, slots 4..15 are 0\n");

  auto arg0Encrypted = db_min_4__encrypt__arg0(cryptoContext, arg0, publicKey);
  dumpCiphertext("ENCRYPTED INPUT (this is what the server sees)",
                 arg0Encrypted);

  auto outputEncrypted = db_min_4(cryptoContext, arg0Encrypted);
  dumpCiphertext("ENCRYPTED RESULT (still unreadable without the key)",
                 outputEncrypted);

  auto actual =
      db_min_4__decrypt__result0(cryptoContext, outputEncrypted, secretKey);

  std::printf("\ndb.min      : %.5f   (expected %.5f)\n", actual, expected);
